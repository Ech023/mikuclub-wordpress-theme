 ...
 * @return string
 */
function print_body_start_component()
{
...
HTML;
    ...
    if (!is_singular())
    {
        ...
        $site_announcement = <<<HTML
            ...
            <div class="speedbar $speedbar_class my-2 p-2 px-3 rounded">
                <span><i class="fa-solid fa-bullhorn"></i> 公告:</span>
                {$announcement_text}
            </div>
...
HTML;
    ...
    if (is_home() || is_single() || is_category() || is_tag())
    {
        ...
        {
            $adsense_component .= '<div class="pop-banner d-none d-md-block text-center my-3 py-2">' . get_theme_option(Admin_Meta::SITE_TOP_ADSENSE_PC) . '</div>';
        }
        if (get_theme_option(Admin_Meta::SITE_TOP_ADSENSE_PHONE_ENABLE))
        {
            $adsense_component .= '<div class="pop-banner d-block d-md-none text-center my-3">' . get_theme_option(Admin_Meta::SITE_TOP_ADSENSE_PHONE) . '</div>';
        }
    }
    ...
    if (is_home() && !get_query_var('paged'))
    {
        ...
        {
            $adsense_component .= '<div class="pop-banner d-none d-md-block text-center my-3 py-2">' . get_theme_option(Admin_Meta::HOME_TOP_ADSENSE_PC) . '</div>';
        }
        if (get_theme_option(Admin_Meta::HOME_TOP_ADSENSE_PHONE_ENABLE))
        {
            $adsense_component .= '<div class="pop-banner d-block d-md-none text-center my-3">' . get_theme_option(Admin_Meta::HOME_TOP_ADSENSE_PHONE) . '</div>';
        }
    }
